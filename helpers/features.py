    ...
    result = [
        {
            ...
            "Holdtime2": data[row_idx + 1][2] - data[row_idx + 1][1],
...
def compute_features(df):
    ...
    # Error Rate %
    total_events = len(df)
    error_events = df[df["key"] == "BACKSPACE"].shape[0]
    ...
    words = get_words(df.to_dict(orient="records"))
    total_time_seconds = (df["timestamp"].max() - df["timestamp"].min()) / 1000
    wpm = len(words) * (60 / total_time_seconds) if total_time_seconds > 0 else 0

    number_of_negative_UD = sum(dig_data["F1"] < 0)
    total_pairs = len(dig_data)
    number_of_negative_UU = sum(dig_data["F2"] < 0)
    neg_ud_percentage = (number_of_negative_UD / total_pairs) * 100
    neg_uu_percentage = (number_of_negative_UU / total_pairs) * 100
    ...
    return {
        ...
        "mean_hold_time1": dig_data["Holdtime1"].mean(),
        "mean_hold_time2": dig_data["Holdtime2"].mean(),
        "mean_f1": dig_data["F1"].mean(),
        "mean_f2": dig_data["F2"].mean(),
        "mean_f3": dig_data["F3"].mean(),
        "mean_f4": dig_data["F4"].mean(),
    }
